...
class DataSet {
    ...
    private void readCSV(String fileName[]){
        ...
        try {
            ...
            values = line.split(",");
            if (!(values.length==2 && values[0].equals("km") && values[1].equals("gastomensal"))) {
                System.err.println("Invalid item, expected \"km,gastomensal\"");
                System.exit(1);
            }
            ...
            while (scanner.hasNext()) {
                ...
                values = line.split(",");
                if (values.length !=2) {
                    System.err.println("Invalid line, expected \"int,int\"");
                    System.exit(1);
                }
            ...
            System.err.println("Error to load the dataset\nErrorType: "+ e.getMessage());
            System.exit(1);
        }
    ...
    public void writeTheta(){
        ...
        try{
            ...
            System.err.printf("Error to save model: %s", e.getMessage());
            System.exit(1);
        }
    }
}
...
class TrainModel {
    ...
    static void plot(DataSet dataSet){
        ...
        try {
            ...
            System.err.println("Error on plot: " + e.getMessage());
            System.exit(1);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < iterations; i++) {
            if (i % 1000 == 0 || i == 0 || i == iterations -1) {
            ...
            for (int j = 0; j < dataSet.trainData.size(); j++) {
                double ePrice = estimatePrice(j, dataSet);
                summ1 += (ePrice - dataSet.trainData.get(j)[1]);
                summ2 += (ePrice - dataSet.trainData.get(j)[1]) * dataSet.trainData.get(j)[0];
            }
